...
export default class EffectInterface {
  ...
  cid_el: HTMLElement;
  val_el: HTMLElement;
  lastEffect: number | null = null;

  constructor(pilot: any, id: string, node: any) {
  ...
  updateAll(data: any, force: boolean = false): void {
    this.updateEffect(data, force);
  }

  updateEffect(data: any, force: boolean = false): void {
  ...
  private to16(float: number): string {
    return Math.floor(float * 15).toString(36);
  }

  private setContent(el: HTMLElement, ct: string): void {
    if (el.innerHTML !== ct) {
      el.innerHTML = ct;
    }
  }
