...
var interface_channel_1 = require("./interface.channel");
var interface_effect_1 = require("./interface.effect");
...
var Mixer = /** @class */ (function () {
    ...
    Mixer.prototype.initializeChannels = function (pilot) {
    ...
    Mixer.prototype.initializeEffects = function (pilot) {
    ...
    Mixer.prototype.install = function (host) {
    ...
    Mixer.prototype.start = function () {
        ...
        setTimeout(function () { _this.effects.limiter.node.toMaster(); }, 2000);
        this.run(''); // Provide an empty string as the default argument
    };
    Mixer.prototype.run = function (msg) {
        if ("".concat(msg).indexOf(';') > -1) {
            var parts = "".concat(msg).split(';');
            for (var id in parts) {
                this.run(parts[id]);
            }
            return;
        ...
        if (msg && "".concat(msg).substr(0, 3).toLowerCase() === 'bpm') {
            this.setSpeed(parseInt(msg.substr(3))); // Convert string to number
        }
        if (msg && "".concat(msg).substr(0, 4).toLowerCase() === 'renv') {
            for (var id in this.channels) {
                this.channels[id].randEnv();
            }
        }
        if (msg && "".concat(msg).substr(0, 4).toLowerCase() === 'rosc') {
            for (var id in this.channels) {
                this.channels[id].randOsc();
            }
        }
        if (msg && "".concat(msg).substr(0, 4).toLowerCase() === 'refx') {
            for (var id in this.effects) {
                this.effects[id].rand(); // Remove the argument
            }
        }
        if (msg && "".concat(msg).substr(0, 5).toLowerCase() === 'reset') {
            this.reset();
        }
    };
    Mixer.prototype.setSpeed = function (bpm) {
        if (bpm < 30) {
            return;
        }
        Tone.Transport.bpm.rampTo(bpm, 4);
        console.log("Changed BPM to ".concat(bpm, "."));
        if (this.pilot && this.pilot.recorder) {
            this.pilot.recorder.el.innerHTML = "".concat(bpm);
        }
    };
